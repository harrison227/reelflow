...
import { CARDS, CLIENTS, ME, NOTIFICATIONS } from '@/lib/mock-data';
    ...
    <aside className="sidebar">
      ...
      {CLIENTS.map((c) => (
        <button
          ...
          type="button"
          className="nav-item"
          onClick={() => router.push(`/?scope=${c.id}`)}
        >
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 10, color: 'var(--fg-mute)', textTransform: 'capitalize' }}>
            {ME.role === 'va' ? 'Virtual Assistant' : ME.role}
          </div>
        </div>
        <Link href="/signin" className="btn ghost sm" title="Sign out">
